...
class _TripBookingFormState extends ConsumerState<TripBookingForm>
  ...
  void initState() {
    ...
    _staggerCtrl = AnimationController(
      ...
      ..forward();
    _anims = List.generate(5, (i) => CurvedAnimation(
      parent: _staggerCtrl,
    ...
    if (widget.booking != null) {
      ...
      if (b.startDateTime != null) {
        ...
        startDate.text = DateFormat("MMM dd, yyyy  •  hh:mm a").format(startDt!);
      }
    ...
    Future.microtask(() {
      ...
      if (widget.booking != null && startDt != null && endDt != null) {
        n.fetchAvailableVehicles(aid, startDt!, endDt!, widget.booking!.tripId!);
  ...
  Future<void> _pickDt(bool isStart) async {
    final date = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(colorScheme: const ColorScheme.light(
          primary: _C.accent, onPrimary: Colors.white,
          surface: _C.surface, onSurface: _C.text1)),
        child: child!),
    );
    if (date == null) return;
    ...
    setState(() {
      if (isStart) { startDt = dt; startDate.text = DateFormat("MMM dd, yyyy  •  hh:mm a").format(dt); }
      else         { endDt   = dt; endDate.text   = DateFormat("MMM dd, yyyy  •  hh:mm a").format(dt); }
    });
...
Future<void> _goAddCustomer() async {
  ...
  Future<void> _save() async {
    ...
    if (startDt == null || endDt == null) { _snack("Select start & end date/time"); return; }
    if (selVehicle == null || selDriver == null || selCustomer == null) {
      _snack("Select vehicle, driver & customer"); return;
    }
    setState(() => _saving = true);
    final bk = TripBooking(
      tripId: widget.booking?.tripId,
      vehicleid: selVehicle!, driverid: selDriver!, customerid: selCustomer!,
      pickuplocation: pickup.text, droplocation: drop.text,
    ...
    final n = ref.read(tripBookingViewModelProvider.notifier);
    if (widget.booking != null) await n.updateTripBooking(widget.booking?.tripId ?? 0, bk);
    else await n.addTripBooking(bk);
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(children: [
                  ...
                  // ── 01  ROUTE ────────────────────────────────────────────
                  _FadeSlide(anim: _anims[0], child: _sectionCard(
                    icon: Icons.route_rounded, label: "Route Details",
                    iconColor: _C.accent, iconBg: _C.accentSoft,
                    badge: "01",
                    child: Column(children: [
                      _inputField(label: "Pickup Location", ctrl: pickup,
                          icon: Icons.trip_origin_rounded,
                          iconColor: _C.green, iconBg: _C.greenSoft),
                      _routeConnector(),
                      _inputField(label: "Drop Location", ctrl: drop,
                          icon: Icons.location_on_rounded,
                          iconColor: _C.red, iconBg: _C.redSoft),
                      _divider(),
                      _inputField(label: "Distance (KM)", ctrl: distance,
                          icon: Icons.straighten_rounded,
                          iconColor: _C.accent, iconBg: _C.accentSoft,
                          keyboard: const TextInputType.numberWithOptions(decimal: true),
                          fmt: [FilteringTextInputFormatter.allow(RegExp(r'[\d.]'))]),
                      const SizedBox(height: 10),
                      _inputField(label: "Fuel Required (L)", ctrl: fuelReq,
                          icon: Icons.local_gas_station_rounded,
                          iconColor: _C.orange, iconBg: _C.orangeSoft,
                          keyboard: const TextInputType.numberWithOptions(decimal: true),
                          fmt: [FilteringTextInputFormatter.allow(RegExp(r'[\d.]'))]),
                      const SizedBox(height: 10),
                      _inputField(label: "Trip Charges", ctrl: charges,
                          icon: Icons.currency_rupee_rounded,
                          iconColor: _C.purple, iconBg: _C.purpleSoft,
                          prefix: "₹  ",
                          keyboard: const TextInputType.numberWithOptions(decimal: true),
                          fmt: [FilteringTextInputFormatter.allow(RegExp(r'[\d.]'))]),
                    ]),
                  ...
                  // ── 02  SCHEDULE ─────────────────────────────────────────
                  _FadeSlide(anim: _anims[1], child: _sectionCard(
                    icon: Icons.calendar_month_rounded, label: "Trip Schedule",
                    iconColor: _C.purple, iconBg: _C.purpleSoft,
                    badge: "02",
                    child: Column(children: [
                      _dateTile(label: "Start Date & Time", ctrl: startDate,
                          icon: Icons.play_circle_rounded,
                          color: _C.green, bg: _C.greenSoft,
                          onTap: () => _pickDt(true)),
                      const SizedBox(height: 10),
                      _dateTile(label: "End Date & Time", ctrl: endDate,
                  ...
                  // ── 03  ASSIGNMENTS ──────────────────────────────────────
                  _FadeSlide(anim: _anims[2], child: _sectionCard(
                    ...
                    child: Column(children: [
                      ...
                      if (startDt == null || endDt == null)
                        _lockedTile("Vehicle", Icons.directions_car_rounded,
                            "Set trip schedule to unlock vehicles")
                      else state.availableVehicles.when(
                        data: (list) {
                          ...
                          if (selVehicle != null && selVehicleLabel == null) {
                            ...
                            if (match.isNotEmpty) {
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (mounted) setState(() => selVehicleLabel = match.first.name ?? '');
                              });
                            }
                          }
                          ...
                          return _customDropTile<int>(
                            ...
                            onSelect: (val, label) => setState(() {
                              selVehicle = val; selVehicleLabel = label;
                            }),
                            hasError: selVehicle == null,
                          );
                        },
                        loading: () => _loadingTile("Fetching available vehicles..."),
                        error: (_, __) => _errorTile("Failed to load vehicles"),
                      ),
                      ...
                      const SizedBox(height: 6),
                      if (startDt == null || endDt == null)
                        _lockedTile("Driver", Icons.person_pin_circle_rounded,
                            "Set trip schedule to unlock drivers")
                      else state.availableDrivers.when(
                        data: (list) {
                          // ── Edit mode: auto-set label from loaded list ──
                          if (selDriver != null && selDriverLabel == null) {
                            final match = list.where((e) => e.driverId == selDriver);
                            if (match.isNotEmpty) {
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (mounted) setState(() => selDriverLabel = match.first.name ?? '');
                              });
                            }
                          }
                          if (list.isEmpty) return _emptyWithAddTile(
                            message: "No drivers available for this schedule",
                          ...
                          return _customDropTile<int>(
                            ...
                            onSelect: (val, label) => setState(() {
                              selDriver = val; selDriverLabel = label;
                            }),
                            hasError: selDriver == null,
                          );
                        },
                        loading: () => _loadingTile("Fetching available drivers..."),
                        error: (_, __) => _errorTile("Failed to load drivers"),
                      ),
                      ...
                      state.fetchCustomerList.when(
                        data: (customers) {
                          WidgetsBinding.instance.addPostFrameCallback((_) {
                            if (widget.booking != null && selCustomer != null) {
                              if (!customers.any((c) => c.customerId == selCustomer))
                                setState(() { selCustomer = null; selCustomerLabel = null; });
                              // ── Edit mode: auto-set label from loaded list ──
                              else if (selCustomerLabel == null) {
                                final match = customers.where((c) => c.customerId == selCustomer);
                                if (match.isNotEmpty) setState(() => selCustomerLabel = match.first.name ?? '');
                              }
                            }
                          });

                          if (customers.isEmpty) return _emptyWithAddTile(
                          ...
                          return _customDropTile<int>(
                            ...
                            onAdd: _goAddCustomer,
                            addLabel: "Add Customer",
                            items: customers.map((e) => _DropItem(
                              value: e.customerId!,
                              label: e.name ?? '',
                              subtitle: "Customer",
                              icon: Icons.person_outline_rounded,
                            ...
                            onSelect: (val, label) => setState(() {
                              selCustomer = val; selCustomerLabel = label;
                            }),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(
              ...
              Text(isEdit ? "Edit Booking" : "New Booking",
  ...
  Widget _sectionCard({
    ...
    required String badge, required Widget child,
  }) {
    return Container(
      decoration: BoxDecoration(
        color: _C.surface,
        ...
        border: Border.all(color: _C.divider),
        boxShadow: [BoxShadow(color: _C.accent.withOpacity(0.06),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: iconColor.withOpacity(0.3),
        ...
        Padding(padding: const EdgeInsets.fromLTRB(14, 14, 14, 16), child: child),
      ]),
    );
  }
  ...
  Widget _inputField({
    ...
    String? prefix, TextInputType? keyboard, List<TextInputFormatter>? fmt,
  }) {
    return TextFormField(
      controller: ctrl, keyboardType: keyboard, inputFormatters: fmt,
      ...
      decoration: InputDecoration(
        ...
        prefixText: prefix,
        prefixStyle: TextStyle(color: iconColor, fontWeight: FontWeight.w700, fontSize: 14),
        prefixIcon: Padding(padding: const EdgeInsets.all(10),
          child: Container(width: 30, height: 30,
            decoration: BoxDecoration(color: iconBg, borderRadius: BorderRadius.circular(8)),
            child: Icon(icon, size: 14, color: iconColor))),
        filled: true, fillColor: _C.surfaceLight,
        ...
        child: Row(children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: has ? [BoxShadow(color: color.withOpacity(0.3),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: const TextStyle(color: _C.text2, fontSize: 11.5)),
            const SizedBox(height: 2),
            Text(has ? ctrl.text : "Tap to select",
              style: TextStyle(
  ...
  Widget _durationBanner() {
    ...
    final h = diff.inHours; final m = diff.inMinutes.remainder(60);
    ...
    return Container(
      ...
      child: Row(children: [
        Icon(inv ? Icons.warning_amber_rounded : Icons.timelapse_rounded,
            size: 15, color: inv ? _C.red : _C.green),
        const SizedBox(width: 8),
        Expanded(child: Text(inv ? "End must be after start" : "Trip Duration",
            style: TextStyle(color: inv ? _C.red : _C.text2,
        ...
        if (!inv)
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: _C.green.withOpacity(0.3), blurRadius: 8)],
            ),
            child: Text(dur, style: const TextStyle(
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Icon(Icons.arrow_forward_ios_rounded, size: 11, color: color.withOpacity(0.6)),
            ]),
          ),
        ),
      ]),
  ...
  Widget _customDropTile<T>({
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      GestureDetector(
        onTap: () => _showDropSheet(items, selected, onSelect, color, bg,
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: hasError && !hasVal
                  ...
                  : hasVal ? color.withOpacity(0.35) : _C.divider,
          ...
          child: Row(children: [
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(11),
                boxShadow: hasVal ? [BoxShadow(color: color.withOpacity(0.3),
                    blurRadius: 8, offset: const Offset(0, 3))] : null,
              ),
              child: Icon(icon, size: 17, color: hasVal ? Colors.white : color),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(hasVal ? placeholder : placeholder,
                  style: const TextStyle(color: _C.text2, fontSize: 11.5)),
              const SizedBox(height: 2),
              Text(
                hasVal ? (selectedLabel ?? placeholder) : "Tap to select",
                style: TextStyle(
            ...
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8)),
              child: Icon(
                hasVal ? Icons.check_rounded : Icons.keyboard_arrow_down_rounded,
                size: 16, color: hasVal ? color : _C.text2.withOpacity(0.5)),
            ),
          ]),
        ),
      ),
  ...
  //  STATE TILES
  // ══════════════════════════════════════════════════════════════════════════
  Widget _assignLabel(String label) => Text(label,
      style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700,
          color: _C.text2, letterSpacing: 0.2));

  Widget _lockedTile(String label, IconData icon, String msg) => Container(
    width: double.infinity,
    padding: const EdgeInsets.all(12),
    decoration: BoxDecoration(
      color: _C.surfaceLight, borderRadius: BorderRadius.circular(12),
      border: Border.all(color: _C.divider)),
    child: Row(children: [
      Container(padding: const EdgeInsets.all(7),
        decoration: BoxDecoration(color: _C.divider, borderRadius: BorderRadius.circular(8)),
  ...
  Widget _loadingTile(String msg) => Container(
    width: double.infinity,
    padding: const EdgeInsets.all(13),
    decoration: BoxDecoration(color: _C.surfaceLight, borderRadius: BorderRadius.circular(12),
        border: Border.all(color: _C.divider)),
    child: Row(children: [
      SizedBox(width: 15, height: 15,
        child: CircularProgressIndicator(strokeWidth: 2,
            color: _C.accent, backgroundColor: _C.accent.withOpacity(0.1))),
      const SizedBox(width: 12),
  ...
  Widget _errorTile(String msg) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(msg, style: const TextStyle(
          color: _C.red, fontSize: 13, fontWeight: FontWeight.w600))),
    ]),
  );
              ...
              decoration: BoxDecoration(
                gradient: _saving
                    ...
                    : const LinearGradient(
                        ...
                        begin: Alignment.topLeft, end: Alignment.bottomRight),
                borderRadius: BorderRadius.circular(13),
                boxShadow: _saving ? [] : [BoxShadow(color: _C.accent.withOpacity(0.35),
                    blurRadius: 12, offset: const Offset(0, 4))],
              ),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                if (_saving)
                  ...
                  Icon(isEdit ? Icons.update_rounded : Icons.check_circle_rounded,
                      color: Colors.white, size: 19),
                const SizedBox(width: 9),
                Text(_saving ? "Saving..." : isEdit ? "Update Trip" : "Save Booking",
...
class _DropSheet<T> extends StatefulWidget {
  ...
  const _DropSheet({required this.items, required this.selected,
      required this.onSelect, required this.color, required this.bg,
      this.onAdd, this.addLabel = "Add New"});
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          width: 36, height: 4,
          decoration: BoxDecoration(color: _C.divider, borderRadius: BorderRadius.circular(4)),
        ),
        ...
        Padding(
          ...
          child: Row(children: [
            Container(
              ...
              child: Icon(widget.items.first.icon ?? Icons.list_rounded,
                  size: 16, color: widget.color),
            ),
            const SizedBox(width: 10),
            const Text("Select Option", style: TextStyle(
                fontSize: 16, fontWeight: FontWeight.w800, color: _C.text1)),
            const Spacer(),
            Text("${widget.items.length} available",
                style: const TextStyle(fontSize: 12, color: _C.text2)),
          ]),
        ),
        const SizedBox(height: 14),
        Padding(
          ...
          child: TextField(
            ...
            style: const TextStyle(fontSize: 14, color: _C.text1, fontWeight: FontWeight.w500),
            decoration: InputDecoration(
              hintText: "Search...",
              hintStyle: const TextStyle(color: _C.text2, fontSize: 13),
              prefixIcon: const Icon(Icons.search_rounded, color: _C.text2, size: 18),
              suffixIcon: _searchCtrl.text.isNotEmpty
                  ? GestureDetector(
                      onTap: () => _searchCtrl.clear(),
                      child: const Icon(Icons.cancel_rounded, color: _C.text2, size: 16))
                  : null,
              filled: true, fillColor: _C.surfaceLight,
          ...
          child: _filtered.isEmpty
              ? Padding(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Container(width: 56, height: 56,
                      ...
                      child: const Icon(Icons.search_off_rounded, color: _C.accent, size: 26)),
                    const SizedBox(height: 12),
              ...
              : ListView.separated(
                  ...
                  separatorBuilder: (_, __) => const Divider(
                      height: 1, indent: 66, color: _C.divider),
                  itemBuilder: (_, i) {
                    ...
                    return InkWell(
                      ...
                      child: AnimatedContainer(
                        ...
                        color: isSelected
                            ? widget.color.withOpacity(0.06) : Colors.transparent,
                        child: Row(children: [
                          Container(
                            width: 42, height: 42,
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(12),
                              boxShadow: [BoxShadow(
                                color: (isSelected ? widget.color : _C.accent).withOpacity(0.25),
                                blurRadius: 8, offset: const Offset(0, 3))],
                            ),
                            child: Center(
                              child: item.icon == Icons.directions_car_rounded
                                  ? Icon(item.icon, color: Colors.white, size: 18)
                                  : Text(_initials(item.label),
                                      style: const TextStyle(color: Colors.white,
                          ...
                          Expanded(child: Column(
                            ...
                            Text(item.label, style: TextStyle(
                              ...
                              color: isSelected ? widget.color : _C.text1)),
                            if (item.subtitle != null) ...[
                              const SizedBox(height: 2),
                              Row(children: [
                                Icon(item.subtitle!.startsWith('0') || item.subtitle!.startsWith('+')
                                    ? Icons.phone_rounded : Icons.badge_rounded,
                                    size: 11, color: _C.text2),
                                const SizedBox(width: 4),
                                Text(item.subtitle!,
                                    style: const TextStyle(fontSize: 11.5,
                                        color: _C.text2, fontWeight: FontWeight.w500)),
                              ]),
                            ],
                          ])),
                          AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              color: isSelected
                                  ? widget.color : _C.surfaceLight,
                              borderRadius: BorderRadius.circular(8),
                              border: Border.all(
                                  color: isSelected ? widget.color : _C.divider),
                            ),
                            child: Icon(
                              isSelected ? Icons.check_rounded : Icons.chevron_right_rounded,
                              size: 15,
                              color: isSelected ? Colors.white : _C.text2.withOpacity(0.4)),
                          ),
                        ]),
                      ),
                    );
