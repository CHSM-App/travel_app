// Reusable driver-report export: data model + PDF/Excel generators + the
// chooser → generate → save → open/share flow. Shared by the fleet-wide
// Driver Report screen and the per-driver history screen so both produce an
// identical document (Overview, Earnings summary, Trips per driver).
...
enum DriverReportExportFormat { pdf, excel }

/// Per-driver figures + the period trips that drive the detail pages.
class DriverReportStat {
  final Drivers driver;
  final List<BookingInfo> trips;

  const DriverReportStat({required this.driver, this.trips = const []});
...
class DriverReportSnapshot {
  final String title; // e.g. "Driver Report" or "Ramesh Report"
  final String periodLabel; // e.g. "Monthly" / "Custom"
  final String dateRangeLabel; // e.g. "01 Jun 2026 - 30 Jun 2026"
  final List<DriverReportStat> stats;
  final double totalReceived;
  final double totalApproved;
  final double totalDriverPay;
  final int activeDrivers;
  final int totalDrivers;
  final int tripCount;

  const DriverReportSnapshot({
    required this.title,
...
// ═══════════════════════════════════════════════════════════════════════════
Future<void> runDriverReportExport(
  BuildContext context,
  DriverReportSnapshot snap,
) async {
  final fmt = await showModalBottomSheet<DriverReportExportFormat>(
    context: context,
  ...
  var ok = false;
  try {
    final isPdf = fmt == DriverReportExportFormat.pdf;
    final bytes =
        isPdf ? await DriverReportPdf.build(snap) : await DriverReportExcel.build(snap);
    final fileName =
...
class _ExportChooserSheet extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _option(
            context,
            format: DriverReportExportFormat.pdf,
          ...
          _option(
            context,
            format: DriverReportExportFormat.excel,
  ...
  Widget _option(
    BuildContext context, {
    required DriverReportExportFormat format,
...
class _ExportReadySheet extends StatelessWidget {
  final String path;
  final DriverReportExportFormat format;
  const _ExportReadySheet({required this.path, required this.format});

  @override
  Widget build(BuildContext context) {
    final isPdf = format == DriverReportExportFormat.pdf;
...
class DriverReportPdf {
  ...
  static pw.Widget _overview(Drivers d) {
    final pairs = <List<String>>[
      ['Name', d.name ?? '-'],
      ['Phone', (d.phone != null && d.phone!.trim().isNotEmpty) ? d.phone!.trim() : '-'],
      ['Address', (d.address != null && d.address!.trim().isNotEmpty) ? d.address!.trim() : '-'],
      ['Licence No', (d.licenceNo != null && d.licenceNo!.trim().isNotEmpty) ? d.licenceNo!.trim() : '-'],
      ['Licence Expiry', _fmtDate(d.licenceExpiry)],
      ['Status', (d.activeStatus != null && d.activeStatus!.trim().isNotEmpty) ? d.activeStatus!.trim() : '-'],
    ];
      ...
      children: [
        pw.TableRow(
          ...
          children: [
            ...
            _cell('Driver Pay', _hStyle, a: pw.TextAlign.right),
          ],
        ),
        ...trips.asMap().entries.map((e) {
          ...
          return pw.TableRow(
            ...
            children: [
              ...
              _cell(_money(t.driverCharges ?? 0), _gStyle,
                  a: pw.TextAlign.right),
            ],
          );
        }),
  ...
  static List<pw.Widget> _driverDetail(
    DriverReportStat s,
    int index, {
    bool pageBreak = true,
  }) {
    ...
    return [
      ...
      pw.Container(
        ...
        child: pw.Row(
          ...
          children: [
            pw.Text(
              index > 0 ? '$index. ${d.name ?? 'Unknown'}' : (d.name ?? 'Unknown'),
              style: pw.TextStyle(
                color: PdfColors.white,
                fontSize: 13,
                fontWeight: pw.FontWeight.bold,
      ...
      pw.Row(
        children: [
          ...
          pw.SizedBox(width: 8),
          _kpi('Driver Pay', _money(s.driverPay), _accent),
          pw.SizedBox(width: 8),
  ...
  static Future<List<int>> build(DriverReportSnapshot snap) async {
    final pdf = pw.Document();
    final dateStr = DateFormat('dd MMM yyyy, hh:mm a').format(DateTime.now());
    final detailed = snap.stats.where((s) => s.hasActivity).toList();
    // A single-driver report skips the redundant summary table and leads with
        ...
        build: (ctx) => [
          ...
          pw.Row(
            children: [
              ...
              pw.SizedBox(width: 10),
              _kpi('Driver Pay', _money(snap.totalDriverPay), _accent),
              pw.SizedBox(width: 10),
              _kpi(
                isSingle ? 'Trips' : 'Active / Total',
                isSingle
                    ? '${snap.tripCount}'
                    : '${snap.activeDrivers} / ${snap.totalDrivers}',
                _dark,
              ),
            ],
          ),
          if (!isSingle) ...[
            ...
            pw.SizedBox(height: 18),
            _sectionBar('Per Driver Summary'),
            pw.SizedBox(height: 8),
            pw.Table(
              border: pw.TableBorder.all(color: _dl, width: 0.5),
              columnWidths: {
              ...
              children: [
                ...
                ...snap.stats.asMap().entries.map((e) {
                  ...
                  return pw.TableRow(
                    ...
                    children: [
                      _cell('${e.key + 1}', _gStyle, a: pw.TextAlign.center),
                      _cell(s.driver.name ?? 'Unknown', _rStyle),
                      _cell(
                        (s.driver.phone != null &&
                                s.driver.phone!.trim().isNotEmpty)
                            ? s.driver.phone!.trim()
                            : '-',
                        _gStyle,
                      ),
                      ...
                      _cell(_money(s.driverPay), _rStyle,
                          a: pw.TextAlign.right),
                    ],
                  );
                }),
                pw.TableRow(
                  ...
                  children: [
                    ...
                    _cell(_money(snap.totalPending), _bStyle,
                        a: pw.TextAlign.right),
                    _cell(_money(snap.totalDriverPay), _bStyle,
                        a: pw.TextAlign.right),
                  ],
                ),
              ],
          ...
          // Per-driver detail. For a single driver the first detail flows on the
          // cover page (no page break); a roster starts each on a new page.
          if (isSingle && detailed.isNotEmpty) ...[
            pw.SizedBox(height: 18),
            ..._driverDetail(detailed.first, 0, pageBreak: false),
          ] else
            for (var i = 0; i < detailed.length; i++)
              ..._driverDetail(detailed[i], i + 1),
        ],
      ),
    );
...
class DriverReportExcel {
  ...
  static String _sheetName(DriverReportStat s, int idx, Set<String> used) {
    var base = (s.driver.name?.trim().isNotEmpty ?? false)
        ? s.driver.name!.trim()
        : 'Driver $idx';
  ...
  static Future<List<int>> build(DriverReportSnapshot snap) async {
    ...
    summaryNum('Total Driver Pay', snap.totalDriverPay);
    meta('Active / Total Drivers',
        '${snap.activeDrivers} / ${snap.totalDrivers}');
    sheet.appendRow([
    ...
    for (var i = 0; i < snap.stats.length; i++) {
      ...
      sheet.appendRow([
        ...
        xls.TextCellValue(s.driver.name ?? 'Unknown'),
        xls.TextCellValue(
          (s.driver.phone != null && s.driver.phone!.trim().isNotEmpty)
              ? s.driver.phone!.trim()
              : '-',
        ),
        ...
        xls.DoubleCellValue(s.driverPay),
      ]);
      r++;
    }
    sheet.appendRow([
      ...
      xls.DoubleCellValue(snap.totalDriverPay),
    ]);
    for (var c = 0; c < headers.length; c++) {
    ...
    final detailed = snap.stats.where((s) => s.hasActivity).toList();
    for (var i = 0; i < detailed.length; i++) {
      _buildDriverSheet(excel, detailed[i], i + 1, used);
    }
  ...
  static void _buildDriverSheet(
    xls.Excel excel,
    DriverReportStat stat,
    int idx,
    Set<String> used,
  ) {
    final d = stat.driver;
    ...
    sheet.appendRow([
      xls.TextCellValue(
        '${d.name ?? 'Unknown'}'
        '${(d.phone != null && d.phone!.trim().isNotEmpty) ? '  (${d.phone!.trim()})' : ''}',
      ),
    ]);
    _style(sheet, 0, r, _titleStyle);
    r++;
    ...
    kv(
      'Licence No',
      xls.TextCellValue(
        (d.licenceNo != null && d.licenceNo!.trim().isNotEmpty)
            ? d.licenceNo!.trim()
            : '-',
      ),
    );
    kv('Licence Expiry', xls.TextCellValue(_fmtDate(d.licenceExpiry)));
    kv(
      'Status',
      xls.TextCellValue(
        (d.activeStatus != null && d.activeStatus!.trim().isNotEmpty)
            ? d.activeStatus!.trim()
            : '-',
      ),
    );
    ...
    kv('Driver Pay', xls.DoubleCellValue(stat.driverPay));
    ...
    if (stat.trips.isEmpty) {
      ...
      for (var i = 0; i < stat.trips.length; i++) {
        ...
        sheet.appendRow([
          ...
          xls.DoubleCellValue(t.driverCharges ?? 0),
        ]);
        r++;
      }
    }
